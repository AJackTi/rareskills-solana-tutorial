...
describe("day_4", () => {
  ...
  it("Input test v2", async () => {
    // Add your test here
    try {
      const tx = await program.methods.limitRangeV2(new anchor.BN(9)).rpc();
      console.log("Your transaction signature", tx);
    } catch (_err) {
      assert.isTrue(_err instanceof AnchorError);
      const err: AnchorError = _err;
    ...
    try {
      const tx = await program.methods.limitRangeV2(new anchor.BN(101)).rpc();
      console.log("Your transaction signature", tx);
    } catch (_err) {
      assert.isTrue(_err instanceof AnchorError);
      const err: AnchorError = _err;
